...
export function EditableWorktreeShellLabel({
  workspaceId,
  agentTabId,
  shellId,
  isActive,
  ...
  const label = useWorktreeTerminalsStore(
    (s) => s.byKey[key]?.find((t) => t.id === shellId)?.label ?? '',
  )
  const renameShell = useWorktreeTerminalsStore((s) => s.renameShell)
  ...
  function commit() {
    renameShell(workspaceId, agentTabId, shellId, draft)
    setEditing(false)
  }
